...
function run_sopf_iv(sdata, model_constructor, optimizer; aux::Bool=true, deg::Int=1, red::Bool=true, kwargs...)
    if aux && red
        return _PMs.run_model(sdata, model_constructor, optimizer, build_sopf_iv_reduced_with_aux; multinetwork=true, kwargs...)
    elseif aux && !red    
        return _PMs.run_model(sdata, model_constructor, optimizer, build_sopf_iv_with_aux; multinetwork=true, kwargs...)
    elseif !aux &&  red
        return _PMs.run_model(sdata, model_constructor, optimizer, build_sopf_iv_reduced_without_aux; multinetwork=true, kwargs...)
    elseif !aux && !red
        return _PMs.run_model(sdata, model_constructor, optimizer, build_sopf_iv_without_aux; multinetwork=true, kwargs...)
    end
end
