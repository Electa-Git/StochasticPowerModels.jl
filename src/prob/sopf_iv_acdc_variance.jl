...
""
function solve_sopf_iv_acdc_variance(data::Dict, model_constructor, optimizer; deg::Int=1, p_size=0, solution_processors=[sol_data_model!], kwargs...)
    ...
    result = _PM.solve_model(data, model_constructor, optimizer, build_sopf_iv_acdc_variance; multinetwork=true, ref_extensions = [_PMACDC.add_ref_dcgrid!, _SPM.add_ref_RES!], solution_processors=solution_processors, kwargs...)
...
""
function build_sopf_iv_acdc_variance(pm::AbstractPowerModel)
    ...
    objective_min_expected_generation_cost_dim_variance(pm)
