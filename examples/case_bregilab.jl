...
f=[ "All_feeder/1136039_1464994_configuration.json",
"All_feeder/65019_74559_configuration.json",
"All_feeder/65028_84566_configuration.json",
"All_feeder/65031_77832_configuration.json"
]
...
for i=1:4
    ...
    hc_det=[]
    for a= [i for i=40:80]
        data  = SPM.build_mathematical_model_single_phase(file, f[i], t_s= a, curt=0 )
    ...
    all_feeder=DataFrame(A=40:80,feeder_sto=hc_sto,feeder_det=hc_det)
    CSV.write("PV_OPF_$(i)0.5.csv",all_feeder)
end
...
for i=1:4
    ...
    hc_det=[]
    for b= [i for i=2:1:2]
        data  = SPM.build_mathematical_model_single_phase(file, f[i], t_s= ts[i], cross_area_fact=b )
        [data["bus"]["$i"]["vmin"]=0.9 for i=1:length(data["bus"])]
        s2 = Dict("output" => Dict("duals" => true))
        result_hc_2= SPM.run_sopf_hc(data, PM.IVRPowerModel, ipopt_solver, aux=aux, deg=deg, red=red; setting=s2)
        ...
        data  = SPM.build_mathematical_model_single_phase(file, f[i], t_s= ts_det[i],cross_area_fact=b )
        result_hc= SPM.run_sopf_hc(data, PM.IVRPowerModel, ipopt_solver, aux=aux, deg=deg, red=red, stochastic=false)
        push!(hc_det,result_hc["objective"])

    end
    all_feeder=DataFrame(A=2:1:2,feeder_sto=hc_sto,feeder_det=hc_det)
    CSV.write("PV_HC_reinforcement_paralle_cable_f$i.csv",all_feeder)
end
#tO= TimerOutput()
...
crd_sample = sample(result_hc_2, "bus", 33, "vs"; sample_size=1000) 
